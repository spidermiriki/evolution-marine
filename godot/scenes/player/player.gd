...
func _input(event: InputEvent) -> void:
	# Clic gauche = attaque pince (PC), vise vers le curseur
...
func _update_camouflage() -> void:
	...
	match species_id:
		...
		"lotte":
			# Camouflage en eaux profondes (>40% de la hauteur)
			is_camouflaged = (global_position.y >= WORLD_H * 0.4) \
				and stealth_cooldown <= 0.0
		"murene":
...
func _handle_movement(delta: float) -> void:
	...
	var can_boost := effective_boost and stamina > 0.0
	if can_boost:
		move_speed *= 1.8
		stamina = max(0.0, stamina - delta * 55.0)
	else:
...
func start_evolve(target_id: String) -> void:
	...
	evolve_duration = float(SpeciesDB.EVOLVE_DURATION.get(tier, 0.9))
...
func _draw() -> void:
	...
	# Arc de griffe (orienté vers facing_angle)
	if claw_anim > 0.0:
